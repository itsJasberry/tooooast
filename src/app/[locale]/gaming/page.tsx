...
// Zmienione na static revalidation
export const revalidate = 1800;

const REGULAR_ARTICLES_PER_PAGE = 6;

// Funkcja do generowania statycznych parametrów
export async function generateStaticParams() {
  // Generujemy parametry dla różnych stron i języków
  // 'en' dla angielskiego (z prefiksem /en)
  // Brak parametru locale dla polskiego (domyślny, bez prefiksu)
  const params = [];

  // Polski (domyślny) - bez locale w params, bo nie ma /pl w URL
  // Next.js będzie generować ścieżki bez prefiksu
  const maxPages = 10; // Możesz dostosować liczbę stron do pre-generowania

  // Dla angielskiego z prefiksem /en
...
const getBadgeVariant = (category: string) => {
  ...
  if (
    categoryLower === "film" ||
    categoryLower === "movie" ||
    categoryLower === "serial" ||
    categoryLower === "tv show"
  )
    return "blue";
      ...
      (page, index, self) =>
        ...
        !(
          (page === 1 &&
            typeof self[index + 1] === "number" &&
            self[index + 1] === 2 &&
            self[index + 2] === "ellipsis") ||
          (page === totalPages &&
            typeof self[index - 1] === "number" &&
            self[index - 1] === totalPages - 1 &&
            self[index - 2] === "ellipsis")
        )
    );
};
      ...
      <div className="flex items-center mb-8 space-x-4 bg-white/80 backdrop-blur-sm rounded-2xl border px-4 py-2">
        ...
        {featuredArticle && (
          <div
            ...
            style={getFeaturedArticleGradientStyle(featuredArticleBadgeVariant)}
          ></div>
        )}
      </div>
      {featuredArticle && (
        <FeaturedArticleComponent
          ...
          locale={locale}
          badgeVariant={featuredArticleBadgeVariant}
        />
      )}
    </>
  );
...
async function PaginatedRegularArticles({
  ...
  if (error) {
    ...
    return (
      <div className="py-10 text-center text-red-500">{t("errorLoading")}</div>
    );
  }
  ...
  const totalPages = Math.ceil(
    totalRegularArticles / REGULAR_ARTICLES_PER_PAGE
  );
    ...
    <>
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {regularArticles.map((article: GamingArticle) => {
          ...
          const titleText =
            locale === "pl" && article.title_pl
              ? article.title_pl
              : article.title;
          const contentText =
            locale === "pl" && article.content_pl
              ? article.content_pl
              : article.content;
          ...
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                href={
                  currentPage > 1
                    ? `/${locale}/gaming?page=${currentPage - 1}`
                    : undefined
                }
...
export default async function GamingNewsPage({
  ...
  const { locale } = params;
  const currentPageToShow = Math.max(
    1,
    parseInt(searchParams?.page || "1", 10) || 1
  );

  const { data: latestArticleForExclusion, error: latestArticleError } =
    await supabase
      .from("gaming_articles")
      .select("id")
      .order("published_at", { ascending: false })
  ...
  if (latestArticleError) {
    console.error(
      "Error fetching latest article ID for exclusion:",
      latestArticleError
    );
    const t = await getTranslations("GamingNewsPage");
    return (
      <div className="container mx-auto px-4 py-8">{t("errorLoading")}</div>
    );
  }
